...
class Question:
    """
    li faudrais comme interface :
    createQuestionFromRepo(filename)
    updateQuestionFromEdited(filevalue)
    """
    ...
    def getcleanjson(self):
        ...
        for key in ["basefiles","grader"]:  # ne pas retirer la clef "soluce"  elle est utilisé par le plgrader de python 
            if key in dico:
                del dico[key]
    ...
    def _createdir(self, pathdir):
        ...
        for name,trad in [('grader','grader.py'),('soluce','soluce.py'),('inputgenerator','inputgenerator.py')]:
            if name in self.dico:
                print("creating",pathdir.joinpath(trad))
