...
def strategy(request, activity):
    ...
    # Add code of previous pl to editor
    if not strat.get_last_answer(strat.get_current_pl(request), request):
        previous = strat.get_previous_pl(request)
        if previous:
            code = strat.get_answer_value(strat.get_last_good_answer(previous, request))
            code += "\n\n"
            code += strat.get_pl_dic(strat.get_current_pl(request))["code"]
            strat.add_to_context(exercise, 'code', code)
