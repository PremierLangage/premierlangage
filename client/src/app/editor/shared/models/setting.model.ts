...
export function loadSettings(): Setting[] {
    let settings: Setting[] = JSON.parse(localStorage.getItem(STORAGE_KEY));
    if (!settings) {
        settings = [
            // Suggestion
            {
            ...
            {
                name: 'renderControlCharacters', group: 'editor', type: SettingTypes.Checkbox, value: true, // default false
...
export function saveSettings(settings: Setting[]) {
 ...
 */
export function settingGroup(settings: Setting[], group: string) {
