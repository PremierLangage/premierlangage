...
dictrans = {
    "execution": "Exécution", "error": "Erreur", "compile": "Compilation ", "feedback": "Retours"
}
...
def checkneeded(needed, studentfile="student.py"):
    """
    check needed est brutal
    il faudrais faire une analyse du code avec l'AST pour
    être sur que les mots clefs sont vraiment des mots clef
    pas des truc ou les loups 'bass' sont transformée en 'bbotom'.
    """
    globalneeded = False
    lneeded = needed.split('|')
    mots = open(studentfile, "r").read()  #
    for x in lneeded:
        reexp = re.compile(x)
        if not reexp.search(mots):
            globalneeded = True
    return globalneeded
...
def compileerror(message):
    ...
    dico_reponse = {
        ...
        "feedback":      "# Erreur de compilation \n\n Le compilateur à détecté une erreur\n\n il "
                         "faut la corriger\n\n" + message,
        "errormessages": "", "other": "", "error": "", "execution": ""
    }
    dodump(dico_reponse)
...
# dépréciée
def plateform(dexec,
              feedback="# Erreur Plateforme \n Un problème de la plateforme\\n parlez en au "
                       "professeur\\n passez à l'exercice suivant"):
    feedback += "\n# Execution \n" + dexec['stdout']
    feedback += "\# Erreurs \n" + dexec['stderr'] + "\n" + error
    dico_reponse = {
        "success":   True, "errormessages": "", "feedback": feedback, "other": "", "error": "",
        "execution": ""
    }
...
def createInputFile(pld, lastgenerated=True):
    ...
    random.randint(4,123))","input":"Toto"}) # ambiguité entre input et inputgenerator
