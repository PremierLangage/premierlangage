...
class Grader:
    ...
    def doOutput(self):
        ...
        if "taboo" in self.pld and checktaboo(self.pld["taboo"], "student"):
            self.fb.adddiv("ftaboo", " la liste des mots taboo " + self.pld[
        ...
        if "needed" in self.pld and checkneeded(self.pld["needed"], "student"):
            self.fb.addFeedback(
                " la liste des mots obligatoires :" + self.pld["needed"] + ". Raté !\n")
            self.fb.addFeedback(" ces mots doivent apparaitres dans votre solution !\n")
            self.fb.success = False
        
        dico_response = {"success":   self.fb.success, "errormessages": "",
                         "feedback":  self.fb.feedback(), "other": "", "error": "XK11",
                         "execution": "", "grade": "1"
        }
    ...
    def compareExpectedOutput(self, expected, i, stdinput=None):
        ...
        if r and c < 3:
            ...
            self.fb.adddiv("ventree output" + str(i), "Entrée :" + self.fb.resultat(
                subnlbybr(stdinput)) + "Attendue:" + self.fb.resultat(
                subnlbybr(expected)) + "Obtenue:" + self.fb.resultat(subnlbybr(expected)))
            self.fb.success = True
        elif r:
            if "nohint" in self.pld:  # don't tel the answer
                self.fb.addOutput(expected)
            else:
                self.fb.adddiv("entree expected obtenu" + str(i), "Entrée: " + self.fb.resultat(
                    subnlbybr(stdinput)) + "\n Attendue: " + self.fb.resultat(
                    subnlbybr(expected)) + "Obtenue:" + self.fb.resultat(subnlbybr(t)))
            ...
            if tmp != 1:
                self.fb.addCompilationError("")
                self.fb.adddiv("errcompil",
                               self.generate_feedback_compilation(self.fb.flags, "error"))
    ...
    def dopltest(self):
        ...
        if 'mode' in self.pld and self.pld['mode'] == 2:
            r, out = self.execute(['python3', '-B', '-m', 'pldoctest', '-f', 'pltest.py'],
                                  instr=None)
        else:
    ...
    def grade(self):
        ...
        if "author" in self.pld:
            self.fb.addFeedback(
                "<H1> Problème exercice mal défini </H1> Contacter l'auteur: " + self.pld[
                    "author"] + "\n Passez à l'exercice suivant.")
        else:
            self.fb.addFeedback(
                "<H1> Problème exercice mal défini </H1>Contacter l'auteur, ou l'administrateur \n Passez à l'exercice suivant.")
        return self.doOutput()
