...
class MoveTestCase(TestCase):
    ...
    def test_move_not_in_directory(self):
        try :
            ...
            self.assertEqual(response.status_code, 200)
            self.assertContains(response, ("Impossible to move 'tests.txt' inside '/../truc': "
    ...
    def test_move_inside_itself(self):
        try :
            ...
            self.assertEqual(response.status_code, 200)
            self.assertContains(response, ("Impossible to move 'extract_test' inside "
                                           "'extract_test': Can't move a directory inside itself."))
        except AssertionError:
            m = list(response.context['messages'])
            if m:
                print("\nFound messages:")
