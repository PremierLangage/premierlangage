$(document).ready(function(){
  ...
  function makeChart(id){
    ...
    for(i=1;i<masterArray.length;i++){
      var noOfBlocks = masterArray[i][1];
      var blocksAdded = 0;
      blockArray = addBlocks(id, noOfBlocks, masterArray, blockArray, nb_line, div_height);
    }
  ...
  function addBlock(id,blockArray,i,timeout){
    ...
    function addBlocks(id, noOfBlocks, masterArray, blockArray, nb_line, div_height){
        ...
        for(b=0;b<noOfBlocks;b++){
            blockArray.push('<div style="width: '+90/nb_line+'%; height: '+div_height/nb_line+'px;" class="block type'+i+blindness+'"><p>'+ masterArray[i][0] +'</p></div>');
        }
        return blockArray;
    }
