...
#~ class ExoTestCase(TestCase):
    ...
    #~ def setUpTestData(self):
        ...
        #~ self.dir = Directory.objects.create(name='cbank', owner=self.user)
        #~ self.pl = load_file(self.dir, 'cbank/exercices/function/polynomial.pl')[0]
        #~ self.sandbox= Sandbox.objects.create(url="http://127.0.0.1:8000/sandbox/?action=execute",name="sanbdboxlocal",priority=200)
    ...
    #~ def test_load_exo_carre(self):
        ...
        #~ response =c.get('/filebrowser/apply_option/?option_h=edit_pl&name_h=polynomial.pl&relative_h=./cbank/exercices/function&type_h=entry',{}, follow=True)
        #~ self.assertEqual(response.status_code,200)
        
    
    #~ def test_reponse_polynomial_false(self):
        #~ c=Client()
        #~ response_eleve = "int polynome(int a, int b, int c){\n\treturn 114;\n}"
        #~ c.force_login(self.user,backend=AUTHENTICATION_BACKENDS[0])
        #~ response =c.get('/filebrowser/apply_option/?option_h=edit_pl&name_h=polynomial.pl&relative_h=./cbank/exercices/function&type_h=entry',{}, follow=True)
        #~ response2 = c.post('/filebrowser/preview_pl/',
        #~ json.dumps({"requested_action":"submit","inputs": {"answer":response_eleve}}),content_type='text/json',follow=True)
        
        #~ self.assertContains(response2,"Aucune erreur d\\u00e9tect\\u00e9.",count=1,status_code=200,msg_prefix="",html=False)
    ...
    #~ def test_reponse_carre_error(self):
        ...
        #~ c.force_login(self.user,backend=AUTHENTICATION_BACKENDS[0])
        #~ response = c.get('/filebrowser/apply_option/?option_h=0&name_h=carre.pl&relative_h=./plbank/python/function&type_h=entry',{}, follow=True)
        #~ response2 = c.post('/filebrowser/preview_pl/',
        #~ json.dumps({"requested_action":"submit","inputs": {"answer":response_eleve}}),content_type='text/json',follow=True)
